...
impl Llm {
    ...
    pub fn openrouter(model: Option<String>, retries: u32, verbose: bool, usage: Arc<Mutex<Usage>>) -> Result<Self> {
        let api_key = std::env::var("OPENROUTER_API_KEY")
            .context("OPENROUTER_API_KEY environment variable not set (export OPENROUTER_API_KEY=...)")?;
        Ok(Llm {
...
fn call_openrouter(api_key: &str, model: Option<&str>, ctx: Option<&str>, task: &str, system: Option<&str>) -> Result<CallResult> {
    ...
    let resp = match result {
        ...
        Err(ureq::Error::Status(code, r)) => {
            ...
            return Err(anyhow!("openrouter responded with status {code}: {}", truncate(&body, 400)));
        }
    ...
    let content = v
        ...
        .and_then(|c| c.as_str())
        .ok_or_else(|| anyhow!("openrouter response missing content: {}", truncate(&v.to_string(), 400)))?;
